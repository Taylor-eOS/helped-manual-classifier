...
class FeatureUtils:
    ...
    def get_global_features(self, block, doc_width=612, doc_height=792, dump=False):
        ...
        if self.global_stats:
            p = self.get_percentile(block['font_size'],
                                    [b['font_size'] for b in self.all_blocks])
            z = (block['font_size'] - self.global_stats['font_size_mean'])\
                      /(self.global_stats['font_size_std']+1e-6)
        ...
        if dump:
            ...
            with open("debug.csv","a") as f:
                ...
                idx = getattr(self,'next_block_index',len(self.training_data))
